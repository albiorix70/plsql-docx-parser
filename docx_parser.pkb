create or replace package body docx_parser as
   ...
   function parse_content_xml(p_content_xml in clob) return t_content_elements is
   ...
   begin
      ...
      for p_rec in (
         select x.p_text from xmltable(
            xmlnamespaces('http://schemas.openxmlformats.org/wordprocessingml/2006/main' as "w"),
            '/w:document/w:body/w:p' passing l_xml
            columns p_text clob path 'string()'
         ) x
      ) loop
      ...
      for p_rec in (
         select x.pnode from xmltable(
            xmlnamespaces('http://schemas.openxmlformats.org/wordprocessingml/2006/main' as "w"),
            '/w:document/w:body/w:p' passing l_xml
            columns pnode xmltype path '.'
         ) x
      ) loop
         for r_rec in (
            select x.t_text from xmltable(
               xmlnamespaces('http://schemas.openxmlformats.org/wordprocessingml/2006/main' as "w"),
               'w:r' passing p_rec.pnode
               columns t_text clob path 'w:t'
            ) x
         ) loop
   ...
   function parse_styles_xml(p_styles_xml in clob) return t_style_list is
   ...
   begin
      ...
      for s_rec in (
         ...
         from xmltable(
            xmlnamespaces('http://schemas.openxmlformats.org/wordprocessingml/2006/main' as "w"),
            '/w:styles/w:style' passing l_xml
            columns
         ...
         begin
            ...
            l_style.is_bold := case when s_rec.b is not null then true else false end;
            l_style.is_italic := case when s_rec.i is not null then true else false end;
            l_style.is_underline := case when s_rec.u is not null and lower(s_rec.u) <> 'none' then true else false end;
   ...
   function extract_text_elements(p_content_xml in clob) return t_content_elements is
   ...
   function extract_paragraphs(p_content_xml in clob) return t_content_elements is
      l_all_elements t_content_elements;
   ...
   function extract_tables(p_content_xml in clob) return t_content_elements is
      l_elements t_content_elements := t_content_elements();
   ...
   function get_formatted_text(p_content_xml in clob) return clob is
   ...
   begin
      ...
      for i in 1..l_elements.count loop
         l_result := l_result || l_elements(i).text_content || chr(10);
      end loop;
   ...
   begin
      ...
      for p_rec in (
         select x.pnode, x.pstyle from xmltable(
            xmlnamespaces('http://schemas.openxmlformats.org/wordprocessingml/2006/main' as "w"),
            '/w:document/w:body/w:p' passing l_xml
            columns pnode xmltype path '.',
         ...
         for r_rec in (
            ...
            l_element.is_bold := case when r_rec.bold is not null then true else null end;
            l_element.is_italic := case when r_rec.italic is not null then true else null end;
            l_element.is_underline := case when r_rec.uval is not null and lower(r_rec.uval) <> 'none' then true else null end;
            ...
            if l_styles.count > 0 then
               ...
               begin
                  ...
                  if (not l_found) and p_rec.pstyle is not null then
                     for s in 1..l_styles.count loop
      ...
      begin
         ...
         l_sql := 'select ' || dbms_assert.simple_sql_name(p_blob_col) ||
                  ' from apex_workflow_files where ' || dbms_assert.simple_sql_name(p_id_col) || ' = :1';
         ...
         exception
            when no_data_found then
               raise_application_error(-20001, 'No row found in apex_workflow_files for ' || p_id_col || '=' || p_id_val);
            when others then
               raise_application_error(-20002, 'Error selecting blob from apex_workflow_files: ' || sqlerrm);
         end;

         -- Try to extract 'word/document.xml' and 'word/styles.xml' using APEX_ZIP
         begin
            -- APEX_ZIP.GET_FILE may return BLOB; adapt if your APEX version differs
            l_doc_blob := apex_zip.get_file(p_zip => l_blob, p_filename => 'word/document.xml');
         exception
            when others then
               l_doc_blob := null;
         end;

         if l_doc_blob is not null then
            -- Convert blob XML to CLOB via XMLTYPE (works if XML encoding is recognized)
            begin
               p_document_xml := xmltype(l_doc_blob).getClobVal();
            exception
               when others then
                  p_document_xml := null;
            end;
         ...
         begin
            l_styles_blob := apex_zip.get_file(p_zip => l_blob, p_filename => 'word/styles.xml');
         exception
            when others then
               l_styles_blob := null;
         end;

         if l_styles_blob is not null then
            begin
               p_styles_xml := xmltype(l_styles_blob).getClobVal();
            exception
               when others then
                  p_styles_xml := null;
            end;
